...
1. **Git Automation Service 구현**
   - 파일: `crates/services/src/services/git_automation.rs`
   - 함수:
     - `sync_before_start()` - git fetch + pull
     - `push_after_complete()` - git push
   - 테스트: `tests/git_automation_test.rs`

2. **Container Service 개선**
   - 파일: `crates/local-deployment/src/container.rs`
   - 수정: `create()` 함수에 auto_sync 추가
   - 테스트: 기존 테스트 업데이트

3. **Database Migration**
   - 파일: `crates/db/migrations/20251117000000_add_git_automation_fields.sql`
   - 테이블: `git_sync_logs`, `task_attempts` 확장

4. **API 엔드포인트 수정**
   - 파일: `crates/server/src/routes/task_attempts.rs`
   - 수정: `start_task_attempt()` 응답에 sync_info 추가

**Deliverables:**
- ✅ Task 시작 시 자동 git pull
- ✅ Git sync 로그 기록
- ✅ 에러 핸들링 및 재시도 로직
...
1. **Complete Task Endpoint 구현**
   - 파일: `crates/server/src/routes/task_attempts.rs`
   - 함수: `complete_task_attempt()`
   - 기능:
     - 자동 커밋 (변경사항 있을 경우)
     - 자동 push
     - PR 자동 생성 (옵션)

2. **GitAutomationService 확장**
   - 함수: `auto_complete()`
   - PR 생성 로직 통합
...
4. **Frontend: Complete Dialog**
   - 파일: `frontend/src/components/dialogs/CompleteTaskDialog.tsx`
   - UI:
     - PR 제목/본문 입력
     - "Create PR" 체크박스
   - API 호출: `completeTaskAttempt()`

**Deliverables:**
- ✅ "Complete" 버튼으로 PR 자동 생성
- ✅ PR 정보 DB 저장
- ✅ Task 상태 자동 전환 (InProgress → InReview)

### Phase 2: Webhook 연동 (1주, 우선순위 2)

#### Week 3: GitHub Webhook

**Tasks:**

1. **Webhook Handler 구현**
   - 파일: `crates/server/src/routes/webhooks.rs`
   - 함수: `handle_github_webhook()`
   - 이벤트 처리:
     - `pull_request.opened`
